/*
Given an array of digital numbers, return a new array of length number containing the last even numbers from the original array (in the same order). The original array will be not empty and will contain at least "number" even numbers.

From https://www.codewars.com/kata/5a431c0de1ce0ec33a00000c/solutions/javascript

For example:

([1, 2, 3, 4, 5, 6, 7, 8, 9], 3) => [4, 6, 8]
([-22, 5, 3, 11, 26, -6, -7, -8, -9, -8, 26], 2) => [-8, 26]
([6, -25, 3, 7, 5, 5, 7, -3, 23], 1) => [6]
*/

/*
function takes in an array of numbers
    -numbers can be positive or negative
    -array will not be empty and contain at least X number of even numbers
function also takes in a number (length to return, X number of even numbers for array)
*/

function evenNumbers(array, length) {
    // filter the array to only include even numbers
    // number % 2 = 0 (remainder of number divided by 2 is 0)
    const evenArray = array.filter(number => number % 2 === 0)
    // slice the filtered array from the end
    // return sliced array
    return evenArray.slice(-length)
}

console.log(evenNumbers([1, 2, 3, 4, 5, 6, 7, 8, 9], 3)) // [4, 6, 8]
console.log(evenNumbers([-22, 5, 3, 11, 26, -6, -7, -8, -9, -8, 26], 2)) // [-8, 26]
console.log(evenNumbers([6, -25, 3, 7, 5, 5, 7, -3, 23], 1)) // [6]
